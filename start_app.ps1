Write-Host "Starting Liberia Application..." -ForegroundColor Cyan
...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "& { $host.UI.RawUI.WindowTitle = 'Liberia Backend'; .\venv\Scripts\python -m hero_lib.app }"

# 3. Start Frontend (Next.js)
Write-Host "Starting Frontend Server (Next.js)..." -ForegroundColor Green
Start-Process powershell -ArgumentList "-NoExit", "-Command", "& { $host.UI.RawUI.WindowTitle = 'Liberia Frontend'; cd 'new UI'; npm run dev }"
